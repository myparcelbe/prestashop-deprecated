...
class MPBpostTools
{
    ...
    public static $thisModule = 'myparcelbpost';
    /** @var string $thisModuleTable */
    public static $thisModuleTable = 'mpbpost';
    ...
    public static function printOrderGridPreference($id, $tr)
    {
        ...
        if (in_array($tr["{$thisModuleTable}_country_iso"], array('NL', 'BE'))) {
            $shippingDaysRemaining = (int) MPBpostDeliveryOption::getShippingDaysRemaining(
                date('Y-m-d 00:00:00'),
    ...
    public static function printCutOffItems($id, $tr)
    {
        ...
        $module = Module::getInstanceByName('myparcelbpost');
        $reflection = new ReflectionClass($module);
        $moduleOverridden = !file_exists(dirname($reflection->getFileName()).'/'.$location);

        return $module->display(
            $moduleOverridden ? _PS_MODULE_DIR_.'myparcelbpost/myparcelbpost.php' : $reflection->getFileName(),
            $location
        );
    }
    ...
    {
        preg_match(MyParcelBpost::ADDRESS_FORMAT_REGEX, AddressFormat::getAddressCountryFormat($idCountry), $fields);
    ...
    public static function getCustomerAddress($idCustomer, $locationCode)
    {
        ...
        $sql->where('a.`alias` = \'mbpost-'.pSQL($locationCode).'\'');
