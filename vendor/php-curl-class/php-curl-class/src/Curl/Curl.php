...
use MPBpostModule\Curl\ArrayUtil;
use MPBpostModule\Curl\Decoder;
class Curl
{
    const VERSION = '8.3.0';
    ...
    public function buildPostData($data)
    {
        ...
        if (isset($this->headers['Content-Type']) && preg_match($this->jsonPattern, $this->headers['Content-Type']) && (is_array($data) || is_object($data) && interface_exists('JsonSerializable', false) && $data instanceof \JsonSerializable)) {
            $data = \MPBpostModule\Curl\json_encode($data);
        } elseif (is_array($data)) {
            // Manually build a single-dimensional array from a multi-dimensional array as using curl_setopt($ch,
            // CURLOPT_POSTFIELDS, $data) doesn't correctly handle multi-dimensional arrays when files are
            // referenced.
            if (\MPBpostModule\Curl\ArrayUtil::is_array_multidim($data)) {
                $data = \MPBpostModule\Curl\ArrayUtil::array_flatten_multidim($data);
            }
    ...
    {
        $this->jsonDecoder = '\\MPBpostModule\\Curl\\Decoder::decodeJson';
    ...
    {
        $this->xmlDecoder = '\\MPBpostModule\\Curl\\Decoder::decodeXml';
    ...
    public function setDefaultDecoder($mixed = 'json')
    {
        if ($mixed === false) {
            ...
            if ($mixed === 'json') {
                $this->defaultDecoder = '\\MPBpostModule\\Curl\\Decoder::decodeJson';
            } elseif ($mixed === 'xml') {
                $this->defaultDecoder = '\\MPBpostModule\\Curl\\Decoder::decodeXml';
            }
        }
    }
    /**
    ...
    public function setUrl($url, $mixed_data = '')
    {
        ...
        if ($this->url === null) {
            $this->url = (string) new \MPBpostModule\Curl\Url($built_url);
        } else {
            $this->url = (string) new \MPBpostModule\Curl\Url($this->url, $built_url);
        }
    ...
    private function parseHeaders($raw_headers)
    {
        ...
        $http_headers = new \MPBpostModule\Curl\CaseInsensitiveArray();
    ...
    {
        $request_headers = new \MPBpostModule\Curl\CaseInsensitiveArray();
    ...
    private function parseResponseHeaders($raw_response_headers)
    {
        ...
        $response_headers = new \MPBpostModule\Curl\CaseInsensitiveArray();
    ...
    private function initialize($base_url = null)
    {
        ...
        $this->setOpt(CURLOPT_RETURNTRANSFER, true);
        $this->headers = new \MPBpostModule\Curl\CaseInsensitiveArray();
