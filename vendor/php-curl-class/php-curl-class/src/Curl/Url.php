...
use MPBpostModule\Curl\StrUtil;
class Url
{
    ...
    public static function removeDotSegments($input)
    {
        ...
        while (!empty($input)) {
            ...
            if (\MPBpostModule\Curl\StrUtil::startsWith($input, '../')) {
                $input = substr($input, 3);
            } elseif (\MPBpostModule\Curl\StrUtil::startsWith($input, './')) {
    ...
    private function absolutizeUrl()
    {
        ...
        if (!($this->relativeUrl === null)) {
            ...
            if (!\MPBpostModule\Curl\StrUtil::startsWith($r['path'], '/')) {
